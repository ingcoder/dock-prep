...
def main():
    ...
    try:
        ...
        OUTPUT_PROTONATED_PQR_FILE = os.path.join(results_folder, f"{PDB_ID}_6_binding_site_protonated.pqr")
        ...
        run_program("MolProbity", OUTPUT_REFINED_FILE, OUTPUT_FLIPPED_H_FILE, verbose=VERBOSE)

        #-----------------------------------------------------------------------
        # Step 5: Clean Molprobity file
        #-----------------------------------------------------------------------
        VERBOSE and add_separator("STEP 5: POSTPROCESSING MOLPPROBITY FILE")   
        run_program("OpenBabel", OUTPUT_FLIPPED_H_FILE, OUTPUT_FLIPPED_H_EDITED_FILE, verbose=VERBOSE)

        #-----------------------------------------------------------------------
        # Step 6: Create PDBQT file for AutoDock Vina
        #-----------------------------------------------------------------------
        VERBOSE and add_separator("STEP 6: CREATING PDBQT FILE FOR AutoDock Vina")  
        run_program("MGLTools", OUTPUT_FLIPPED_H_EDITED_FILE, OUTPUT_PDBQT_DOCKING_FILE, verbose=VERBOSE)
